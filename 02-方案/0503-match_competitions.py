...
def parse_profile_template(filepath: str) -> dict:
    """解析用户画像模板文件（HTML表单生成的Markdown格式），提取字段到 dict。

    支持三种格式：
    1. 文本字段: - **学校**: 湖北师范大学文理学院
    2. 单选项:   - **年级**: [x] 大一          (提取 [x] 后的值)
    3. 多选组:   - **兴趣领域**（可多选）:      (后续 [x] 行归入该组)
                   - [x] AI/大模型
    4. 编号列表: 1. 项目描述                   (如代表性项目)
    5. 节标题直连: ## 二、参赛目标 ⭐必填      (无 bold 标签时从节标题映射)
    留空字段(_或___)视为未填写。
    """
    import re
    ...
    _FIELD_MAP = {
        ...
        "编程语言": "programming_langs", "补充说明": "other_skills",
        "理想目标": "ideal_goal", "每周可投入": "time_commitment",
        "寒暑假可集中备赛": "summer_winter",
        "赛事级别": "preference", "个人/团队": "team_preference",
        "有指导老师": "has_advisor", "接受跨校组队": "can_cross_school",
        "比赛周期": "preferred_duration", "比赛形式": "preferred_format",
        "报名费": "fee_budget", "语言": "language_pref",
        "最高获奖": "past_highest_award",
        "最低接受": "min_award", "策略": "strategy",
    ...
    def _best_match(text: str, mapping: dict):
        """在 mapping 中查找最匹配的 key（优先精确匹配，其次前缀匹配）。"""
        if text in mapping:
            return mapping[text]
    ...
    def _is_empty(val: str) -> bool:
        """判断值是否为未填写。"""
        v = val.strip()
        return not v or v in ("___", "_", "其他: ___", "其他:", "其他：___")
    ...
    def _flush_group():
        nonlocal group_key, group_values, group_type
        if group_key and group_values:
            result[group_key] = group_values
        group_key = None
        group_values = []
        group_type = "checkbox"
    ...
    for line in lines:
        stripped = line.strip()
        if not stripped or (stripped.startswith('#') and not stripped.startswith('##')) or stripped.startswith('---') or stripped.startswith('>'):
            continue

        # 节标题：## 二、参赛目标 ⭐必填
        sec_m = re.match(r'##\s+[一二三四五六七八九]、(.+?)(?:\s+[⭐△]+.*)?$', stripped)
        if sec_m:
            _flush_group()
            sec_title = sec_m.group(1).strip()
            sk = _best_match(sec_title, _SECTION_MAP)
            if sk:
                group_key = sk
                group_values = []
                group_type = "checkbox"
        ...
        # bold 标签行：- **key**（可选说明）: value
        bold_m = re.match(r'-\s*\*\*(.+?)\*\*(?:\s*（[^)]*）)?\s*:?\s*(.*)', stripped)
        if bold_m:
            key_raw = bold_m.group(1).strip()
            value_raw = bold_m.group(2).strip()
            _flush_group()

            if not value_raw:
                gk = _best_match(key_raw, _GROUP_MAP)
                if gk:
                    group_key = gk
                    group_values = []
                    group_type = "numbered" if gk == "representative_projects" else "checkbox"
                continue

            value_raw = re.sub(r'^\[x\]\s+', '', value_raw, flags=re.IGNORECASE).strip()
            if _is_empty(value_raw):
                continue

            fk = _best_match(key_raw, _FIELD_MAP)
            if fk == "has_portfolio":
                if "没有" in value_raw:
                    result["has_portfolio"] = False
                elif "有" in value_raw:
                    result["has_portfolio"] = True
                    link_m = re.search(r'链接[：:]\s*(.+)', value_raw)
                    if link_m:
                        result["portfolio_link"] = link_m.group(1).rstrip('）)')
            elif fk:
                result[fk] = value_raw
            continue

        # checkbox 行：- [x] AI/大模型
        cb_m = re.match(r'\s*-\s*\[(x|X|√)\]\s+(.+?)$', stripped)
        if cb_m:
            checked = cb_m.group(2).strip()
            if group_key and group_type == "checkbox" and not _is_empty(checked):
                group_values.append(checked)
            continue
        ...
        if num_m:
            ...
            if group_key and group_type == "numbered" and not _is_empty(text):
...
# 画像构建辅助函数
# ============================================================
def _build_profile(extended_profile: dict, args) -> dict:
    """从画像模板 + CLI参数构建完整的 profile dict。

    模板字段优先，CLI --xxx 参数兜底。
    list 类型字段（多选组）自动转为中文顿号连接。
    """
    def _get(key, default=""):
        val = extended_profile.get(key, default)
    ...
    cli_interests = args.interests or ""

    profile = {
        "school":        _get("school", args.school),
        "major":         _get("major", args.major),
        "grade":         _get("grade", args.grade),
        "interests":     _get("interests", cli_interests),
        "skills":        _get("interests") or _get("tech_directions"),
        "programming_langs": _get("programming_langs"),
        "tech_directions":   _get("tech_directions"),
        "tools":             _get("tools"),
        "other_skills":      _get("other_skills"),
        "goals":             _get_list("goals"),
        "requirements":      args.requirements,
        "time_commitment":   _get("time_commitment"),
        "available_months":  _get("available_months"),
        "summer_winter":     _get("summer_winter"),
        "preference":           _get("preference"),
        "team_preference":      _get("team_preference"),
        "preferred_duration":   _get("preferred_duration"),
        "preferred_format":     _get("preferred_format"),
        "fee_budget":           _get("fee_budget"),
        "language_pref":        _get("language_pref"),
        "has_advisor":       _get("has_advisor"),
        "can_cross_school":  _get("can_cross_school"),
        "avoid_types":       _get("avoid_types"),
        "past_highest_award":     _get("past_highest_award"),
        "representative_projects": _get("representative_projects"),
        "has_portfolio":     extended_profile.get("has_portfolio", None),
        "portfolio_link":    _get("portfolio_link"),
        "min_award":   _get("min_award"),
        "ideal_goal":  _get("ideal_goal"),
        "strategy":    _get("strategy"),
        "has_lab":          _get("has_lab"),
        "join_school_team": _get("join_school_team"),
        "need_teammate":    _get("need_teammate"),
        "gpa_rank":                _get("gpa_rank"),
        "past_national":           "",
        "past_provincial":         "",
        "past_school":             "",
        "preferred_team_size":     "",
        "secondary_goals":         _get_list("secondary_goals"),
    }

    highest = profile.get("past_highest_award", "")
    if "国际" in highest:
        profile["past_national"] = "国际级获奖"
    elif "国家" in highest:
        profile["past_national"] = "国家级获奖"
    elif "省" in highest:
        profile["past_provincial"] = "省级获奖"
    elif "校" in highest:
        profile["past_school"] = "校级获奖"
...
def main():
    ...
    if args.analyze or args.batch:
        if not args.school or not args.major or not args.grade:
            ...
            sys.exit(1)
        # V1.4+: 合并画像模板 + CLI参数
        profile = _build_profile(extended_profile, args)
        print(f"""
╔══════════════════════════════════════════════════════════╗
