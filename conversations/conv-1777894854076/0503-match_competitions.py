#!/usr/bin/env python3
"""
竞赛选赛助手 V1.6 (CLI本地终端) — 全面收集 + 常识库校验 + 主动验证 + 手动分析
...
def enrich_with_facts(competition: dict) -> dict:
    ...
    # 官网：常识库优先
    known_url = facts.get("official_url", "")
    if known_url and known_url != competition.get("official_url", ""):
...
│ 官网:     {official_url[:52] if official_url else '未找到'}{url_status}
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="竞赛选赛助手 V1.6 (CLI本地终端) — 全面收集+主动验证")
...
""")
    ...
    print("""
╔══════════════════════════════════════════════════════════╗
║        🔍 竞赛选赛助手 V1.6 (CLI本地终端) — 全面收集+主动验证     ║
║  画像 → 搜索 → 匹配 → 常识库校验 → 交叉验证 → 自审查        ║
╚══════════════════════════════════════════════════════════╝
""")
    print(f"👤 {profile['school']} | {profile['major']} | {profile['grade']}")
